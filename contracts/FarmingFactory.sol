...
contract FarmingFactory {
    event FundDistributorCreated(
      address indexed addr,
      address indexed reward
    );
    event MasterChefCreated(
      address indexed mcAddr,
      address _reward,
      address fdAddr
    );
    ...
    function createMasterChef(address _reward, address _fundDistributor) public returns (address mc) {
      mc = address(new MasterChef(IERC20(_reward), IFundDistributor(_fundDistributor), ITokenUtilityModule(address(0))));
